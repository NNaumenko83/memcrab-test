...
const Matrix: React.FC = () => {
    const { matrix, incrementCell, deleteRow, rowsQuantity, columnsQuantity } = useMatrixContext()
    ...
    return (
        <Container>
            ...
            {matrix.map((row, rowIndex) => {
                const rowSum = row.reduce((acc, c) => acc + c.amount, 0)
                ...
                return (
                    ...
                    >
                        ...
                        {row.map((cell, colIndex) => {
                            const value = showPercent ? ((cell.amount / rowSum) * 100).toFixed(1) + "%" : cell.amount

                            return (
                                <div
                                    key={cell.id}
                                    className={`${css.cell} ${highlightedIds.includes(cell.id) ? css.highlighted : ""} ${showPercent && css.row__highlighted}`}
